...
function renderAnimatedDot(flow, index) {
  const routed = pathFor(flow);
...
function renderSvg() {
  const flows = asArray(dataflow.flows);
...
${dataflow.stages.map(renderStage).join('\n\n')}
        ...
        <!-- Flow paths -->
${flows.map(renderFlowPath).join('\n')}

        <!-- Animated flowing dots -->
${flows.map((flow, i) => renderAnimatedDot(flow, i)).join('\n')}

        <!-- Nodes -->
${[...nodes.values()].map(renderNode).join('\n\n')}

        <!-- Flow labels -->
${flows.map(renderFlowLabel).join('\n')}
